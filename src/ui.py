...
def render_lane_selector() -> list[str]:
    ...
    for idx, lane in enumerate(lane_options):
        ...
        with cols[idx]:
            ...
            if st.button("Selected" if selected else "Select", key=f"lane_toggle_{idx}", use_container_width=True):
    ...
    st.markdown('<div class="selection-row-wrap">', unsafe_allow_html=True)
    c1, c2 = st.columns([0.80, 0.20])
    with c1:
        st.markdown(
            f'<div class="lane-selection-summary"><span class="summary-dot">✓</span><strong>{html.escape(summary_label)}</strong><span>{html.escape(caption)}</span></div>',
            unsafe_allow_html=True,
        )
    with c2:
        clear_label = "Reset All" if count != len(lane_options) else "Clear All"
        if st.button(clear_label, key="lane_clear_all", use_container_width=True):
            st.session_state.scan_lanes = [] if count == len(lane_options) else lane_options.copy()
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
